...
export default function MenuItemCard({ item, onUpdate, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className={`group/card relative flex flex-col overflow-hidden rounded-[2rem] md:rounded-2xl bg-white border border-stone-200 transition-all duration-500 hover:shadow-[0_40px_80px_rgba(28,25,23,0.06)] md:hover:shadow-[0_20px_40px_rgba(28,25,23,0.06)] hover:-translate-y-1.5 md:hover:-translate-y-1 ${
    ...
    >
      <div className="relative h-48 md:h-36 w-full overflow-hidden">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-1.5 pointer-events-none z-10">
          <div className="flex items-center gap-2 md:gap-1.5 rounded-full bg-white/95 backdrop-blur-md px-4 md:px-3 py-2 md:py-1.5 text-stone-900 shadow-xl border border-white/50 pointer-events-auto">
      ...
      <div className="flex flex-col gap-2 p-4">
        <div className="flex justify-between items-start gap-3">
          <h4 className="flex-grow font-['Outfit',sans-serif] text-2xl md:text-lg font-bold text-stone-950 leading-[1.1] md:leading-[1.2] italic">
            {item.name}
          </h4>
          <div className="flex-shrink-0">
            <span className="text-2xl md:text-lg font-bold text-emerald-600 font-['Outfit',sans-serif]">
              ${priceDisplay}
            </span>
          </div>
        </div>
        ...
        <textarea
          ...
          className="w-full bg-stone-50 rounded-lg border border-stone-100 px-3 py-2 text-xs font-medium leading-relaxed text-stone-500 italic resize-none outline-none focus:text-stone-900 focus:bg-white focus:border-emerald-200 transition-colors" rows={1} style={{ minHeight: '36px' }}
        />

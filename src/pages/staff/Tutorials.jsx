...
/**
 * Staff Tutorials - KDS workflow basics
 * 1 essential video for kitchen staff
 */
const STAFF_TUTORIALS = [
  {
    id: 'kds-workflow',
    title: 'KDS Workflow',
    description: 'How to accept orders, mark items done, and send to customers',
    duration: '3:45',
    category: 'Operations'
  }
]
...
export default function StaffTutorials() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 pb-20">
        <div className="mb-6">
          ...
          <p className="text-slate-600 dark:text-slate-400">
            Quick guides to help you work efficiently in the kitchen
          </p>
        </div>

        <div className="space-y-4">
          {STAFF_TUTORIALS.map((tutorial) => (
            <TutorialCard
              key={tutorial.id}
              tutorial={tutorial}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-800 dark:text-emerald-200">
                We're creating tutorial videos. Your manager will let you know when they're ready!
              </p>
            </div>
          </div>
        </div>
      ...
      <BackendNav role="staff" useRoutes={true} />
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 max-w-2xl w-full">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
                {STAFF_TUTORIALS.find(t => t.id === selectedTutorial)?.title}
              </h2>
              <button
                onClick={handleClose}
